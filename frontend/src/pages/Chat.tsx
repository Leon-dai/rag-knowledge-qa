  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#fff', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {currentSession && (
          <div style={{ textAlign: 'center', padding: '16px 24px 8px', background: '#fff' }}>
            <Text style={{ fontSize: 16, fontWeight: 500 }}>{currentSession.title}</Text>
          </div>
        )}
          ...
          {loading && messages.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
              <Empty description="开始提问吧">
                <Button type="primary" icon={<PlusOutlined />} onClick={handleCreateSession}>开启新对话</Button>
              </Empty>
            </div>
          ) : (
            <div style={{ maxWidth: 800, margin: '0 auto' }}>
              ...
              {streamingContent && (
                <MessageBubble message={{ id: 'streaming', role: 'assistant', content: streamingContent, citations: null, created_at: '' }} isStreaming />
              )}
            ...
            <div style={{ marginBottom: 12, display: 'flex', gap: 8 }}>
              {SEARCH_MODES.map((mode) => (
                <Tooltip key={mode.value} title={mode.tooltip}>
                  <Button type={searchMode === mode.value ? 'primary' : 'default'} size="small" icon={mode.icon}
                    onClick={() => setSearchMode(mode.value as 'local' | 'web' | 'mixed')} style={{ borderRadius: 20 }}>
                    {mode.label}
                  </Button>
                </Tooltip>
              ))}
            </div>
            <div style={{ background: '#f5f5f5', padding: '12px 16px', borderRadius: 12, border: '1px solid #e8e8e8' }}>
